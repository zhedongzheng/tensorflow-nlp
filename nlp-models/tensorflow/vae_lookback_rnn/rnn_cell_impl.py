...
class AttnGRUCell(_LayerRNNCell):
  ...
  def __init__(self,
               ...
               name=None):
    super(AttnGRUCell, self).__init__(_reuse=reuse, name=name)
  ...
  def attend(self, keys, query):
    values = keys
    query = array_ops.expand_dims(query, -1)
    align = math_ops.matmul(keys, query)
    align = nn_ops.softmax(array_ops.squeeze(align, -1))
    return array_ops.squeeze(math_ops.matmul(array_ops.transpose(
      values, [0,2,1]), array_ops.expand_dims(align, 2)), -1)
